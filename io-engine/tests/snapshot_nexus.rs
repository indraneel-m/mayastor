...
use io_engine::{
    bdev::{device_open, device_create},
...
use io_engine::sleep::mayastor_sleep;
...
async fn launch_instance() -> (ComposeTest, String) {
    ...
    ms1.pool.create_pool(CreatePoolRequest {
    ...
    ms1.replica.create_replica(CreateReplicaRequest {
            name: replica_name(),
            uuid: replica_uuid(),
            pooluuid: pool_uuid(),
            size: 8 * 1024 * 1024,
...
async fn test_replica_handle_snapshot() {
    ...
    let mut ms1 = conn.grpc_handle("ms1")
        .await
    ...
    ms.spawn(async move {
        ...
        println!("@@@@@@@@@@@@@@@@  Creating a snapshot at {}", device_name);
        handle
        ...
        //println!("@@@@@@@@@@@@@@@@  Sleeping ...");
        //mayastor_sleep(Duration::from_secs(3)).await;
        //println!("@@@@@@@@@@@@@@@@  Got woken up !!!");
    })
    .await;
